...
namespace travel {
    ...
    private:
        ...
        pcl::PointCloud<PointType> new_ptCloud_tgfwise_nonground_;
    ...
    public:
        ...
        void setParams(const double max_range, const double min_range, const double resolution, 
            ...
            new_ptCloud_tgfwise_nonground_.clear();
            new_ptCloud_tgfwise_nonground_.reserve(PTCLOUD_SIZE);
		...
		bool esti_plane(Eigen::Matrix<float, 4, 1> &pca_result, const pcl::PointCloud<PointType> &point_cloud, const float &threshold)
		{
			const int NUM_MATCH_POINTS = point_cloud.size();  // 假设你使用点云大小作为匹配点数量

			if (point_cloud.size() < NUM_MATCH_POINTS) {
				// 点云中的点数不足以进行平面拟合
				return false;
			}

			Eigen::Matrix<float, Eigen::Dynamic, 3> A(NUM_MATCH_POINTS, 3);
			Eigen::Matrix<float, Eigen::Dynamic, 1> b(NUM_MATCH_POINTS, 1);
			A.setZero();
			b.setOnes();
			b *= -1.0f;

			// 将点云的 x, y, z 坐标填充到矩阵 A 中
			for (int j = 0; j < NUM_MATCH_POINTS; j++)
			{
				A(j, 0) = point_cloud.points[j].x;
				A(j, 1) = point_cloud.points[j].y;
				A(j, 2) = point_cloud.points[j].z;
			}
			...
			pca_result(3) = 1.0 / n;
			/*
			// 检查所有点与拟合平面的距离，若超过阈值则返回 false
			for (int j = 0; j < NUM_MATCH_POINTS; j++)
			{
				if (fabs(pca_result(0) * point_cloud.points[j].x +
				         pca_result(1) * point_cloud.points[j].y +
				         pca_result(2) * point_cloud.points[j].z +
        ...
        void estimateGround(const pcl::PointCloud<PointType>& cloud_in,
            ...
            //将三角网格字段（TriGridField）中的点云数据按照地面（ground）、非地面（nonground）、障碍物（obstacle）和离群点（outliers）进行分类和分割
			...
			if (esti_plane(pca_result, ptCloud_tgfwise_ground_, 0.1)) {
				float a = pca_result(0);
				float b = pca_result(1);
				float c = pca_result(2);
				float d = pca_result(3);
				float norm = sqrt(a * a + b * b + c * c);
				for (auto point : ptCloud_tgfwise_nonground_.points) {
					float distance = fabs(a * point.x +
                          b * point.y +
                          c * point.z +
                          d) / norm;
					// 如果距离小于 0.05，则认为该点是地面点，将其加入到 ground 点云中
					if (distance < 0.1) {
						ptCloud_tgfwise_ground_.push_back(point);
					} else {
						// 否则保留为非地面点
						new_ptCloud_tgfwise_nonground_.push_back(point);
					}
				}
			}
			else{cout<<"ground error"<<endl;}
            cloudGround_out = ptCloud_tgfwise_ground_;
            cloudNonground_out = new_ptCloud_tgfwise_nonground_;
            new_ptCloud_tgfwise_nonground_.clear();
            //cloudNonground_out = ptCloud_tgfwise_obstacle_;
    ...
    private:
		...
		//使用节点的法向量和距离阈值对点进行分类，分别输出地面点、非地面点、障碍物点和离群点
	void segmentNodeGround(const TriGridNode<PointType>& node_in,
		...
		// 判断点云数量是否小于3
		if (node_in.ptCloud.points.size() < 8) {
		    // 如果点云数量小于3，将所有点加入到node_outlier_out
		    node_outlier_out = node_in.ptCloud;
		    return;
		}
        ...
        void segmentTGFGround(const TriGridField<PointType>& tgf_in, 
            ...
            for (int s_i = 0; s_i < tgf_in[r_i][c_i].size(); s_i++) {
                ...
                ptCloud_nodewise_outliers_.clear();
                ptCloud_nodewise_obstacle_.clear();
                //如果当前节点的类型为地面，调用 segmentNodeGround 函数进一步对该节点的点云数据进行分割
                if (tgf_in[r_i][c_i][s_i].node_type == GROUND) {
                ...
                //如果节点不是地面节点，则将该节点的点云直接归类为非地面和障碍物点云
                else {
					if (tgf_in[r_i][c_i][s_i].ptCloud.size() > 8) {
						ptCloud_nodewise_nonground_ = tgf_in[r_i][c_i][s_i].ptCloud;
						ptCloud_nodewise_obstacle_ = tgf_in[r_i][c_i][s_i].ptCloud;
					}
					else {
						ptCloud_nodewise_outliers_ = tgf_in[r_i][c_i][s_i].ptCloud;
					}
                }
